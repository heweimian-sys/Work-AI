...
hermes plugins enable feishu-cli-router

$jobText = hermes cron list --all | Out-String
$jobBlocks = [regex]::Matches($jobText, "(?ms)^\s{2}([a-z0-9]+) \[.*?(?=^\s{2}[a-z0-9]+ \[|\z)")
$newsBlock = $jobBlocks | Where-Object { $_.Value -match "Name:\s+(?:AI 最新项目追踪日报|AI 资讯日报（确定性脚本）)" } | Select-Object -First 1
$newsId = if ($newsBlock) { $newsBlock.Groups[1].Value } else { $NewsJobId }
if (-not $Deliver -and $newsBlock -and $newsBlock.Value -match "Deliver:\s+(\S+)") { $Deliver = $Matches[1] }
...
$reviewBlock = $jobBlocks | Where-Object { $_.Value -match "Name:\s+每晚工作复盘（候选记忆）" } | Select-Object -First 1
if ($reviewBlock) {
    hermes cron edit $reviewBlock.Groups[1].Value --schedule "30 21 * * *" --script "nightly_review.py" --no-agent --deliver $Deliver
} else {
